...
DIR=`grep data_path /home/arducam/bin/config.py | sed -n "s/^.*'\(.*\)'.*$/\1/p"`"/Studio"
...
attractorON=`grep attractorON_gpio /home/arducam/bin/config.py | sed 's/[^0-9]//g'`
panelOFF=`grep studioOFF_gpio /home/arducam/bin/config.py | sed 's/[^0-9]//g'`
panelON=`grep studioON_gpio /home/arducam/bin/config.py | sed 's/[^0-9]//g'`
...
libcamera-still -n --datetime --autofocus-mode manual --lens-position 7 --sharpness 2 
...
echo $location", "$serialNum", Studio, "$newFilename", OwlSight, 4624x3472" >> /home/arducam/data/curdat/dataLog.csv
